...
#ifndef ALLOC
# define ALLOC(size) (Tag_Malloc(size, MEM_FILE))
#endif
#ifndef TRYFREE
# define TRYFREE(p) {if (p) Tag_Free(p);}
#endif
...
extern zipFile ZEXPORT ZIPW_Open (pathname, append)
    const char *pathname;
    int append;
{
    zip_internal ziinit;
...
extern int ZEXPORT ZIPW_AddFileInZip (file, filename, zipfi, 
...
extern int ZEXPORT ZIPW_WriteFileInZip (file, buf, len)
    zipFile file;
    const voidp buf;
    unsigned len;
{
...
extern int ZEXPORT ZIPW_CloseFileInZip (file)
    zipFile file;
{
    zip_internal* zi;
    int err=ZIP_OK;
...
extern int ZEXPORT ZIPW_Close (file, global_comment)
    zipFile file;
