...
local uLong unzlocal_SearchCentralDir(FILE *fin)
{
	...
	buf = (unsigned char*)Tag_Malloc(BUFREADCOMMENT+4, MEM_ZIP);
	if (buf==NULL)
		return 0;
...
size_t File_Unzip_Read(void *ptr, size_t size, size_t nmemb, file_t *file)
{
	return unzReadCurrentFile (file->data, ptr, size * nmemb);
}

size_t File_Unzip_Write (const void *ptr, size_t size, size_t nmemb, struct file_s *stream)
{
	return 0;
}
...
int File_Unzip_Close (file_t *file)
{
	return (unzCloseCurrentFile (file->data) == UNZ_OK);
}

static int zip_str_equal(gconstpointer v, gconstpointer v2)
{
	//Console_Printf("hash - comparing %s with %s\n", (char *)v, (char *)v2);
...
static gboolean _destroyKeyValue(gpointer key, gpointer value, gpointer user_data)
{
	g_free(key);
	return true;
}
	...
	unzFile file;
{
	...
	g_hash_table_foreach_remove(s->filenameHash, _destroyKeyValue, NULL);
	
	g_hash_table_destroy(s->filenameHash);
...
bool ZIP_FileExists	(unzFile archive, const char *filename)
{
	gpointer file_entry;
	char *lowerfilename;
	unz_s* s;
	...
	if (file_entry)
	{
		//Console_Printf("found %s in this archive\n", filename);
		return true;
	}

	//Console_Printf("%s does not appear to be in this archive\n", filename);
	//Console_DPrintf("exists() on %s failed\n", filename);
	return false;
}


file_t *ZIP_OpenFile(unzFile archive, const char *filename, char *mode)
{
	int err;
	int pos, size;
	unz_s unz_backup;
	...
	{
		Console_DPrintf("Unknown mode '%s' in ZIP_OpenFile\n", mode);
		return NULL;
	}
	s=(unz_s*)archive;
	...
	if (file_entry)
	{
		...
		filestruct = Tag_Malloc(sizeof(file_t), MEM_ARCHIVE);
		memset(filestruct, 0, sizeof(file_t));
		filestruct->file = (void *)pos;
		filestruct->data = (void *)archive;
		filestruct->read = File_Unzip_Read;
		filestruct->write = File_Unzip_Write;
		filestruct->close = File_Unzip_Close;
		filestruct->eof = File_Unzip_Eof;
		filestruct->flush = NULL;
		filestruct->tell = NULL;
		filestruct->seek = NULL;
		File_AddOpenFileTracking(filename, filestruct);
		
		size = s->cur_file_info.uncompressed_size;

		buf = Tag_Malloc(size, MEM_FILEBUFFER);
		File_Unzip_Read(buf, size, 1, filestruct);
		
		bufFilestruct = File_NewBufferFile();
		File_Buffered_Import(bufFilestruct, buf, size);
		File_FileTrackingRename(bufFilestruct, filename);
		
		File_Close(filestruct);

		Mem_Copy(s, &unz_backup, sizeof(unz_s));
		return bufFilestruct;
	}
	...
	unzFile file;
{
	...
	pfile_in_zip_read_info = (file_in_zip_read_info_s*)
									    Tag_Malloc(sizeof(file_in_zip_read_info_s), MEM_ZIP);
	if (pfile_in_zip_read_info==NULL)
		return UNZ_INTERNALERROR;

	pfile_in_zip_read_info->read_buffer=(char*)Tag_Malloc(UNZ_BUFSIZE, MEM_ZIPBUFFER);
	...
	{
		Tag_Free(pfile_in_zip_read_info);
		return UNZ_INTERNALERROR;
	}
	...
	unzFile file;
{
	...
	Tag_Free(pfile_in_zip_read_info->read_buffer);
